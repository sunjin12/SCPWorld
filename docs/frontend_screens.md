...
- **표시**: AppBar "SELECT PERSONA" + 3개 카드:
  - **Dr. [REDACTED]** (researcher) — 수석 연구원, `[DEFAULT]` 배지
  - **Agent [REDACTED]** (agent) — 현장 요원
  - **SCP-079** (scp079) — 구식 AI
- **사용자 액션**:
...
| stage | 한국어 라벨 |
|-------|-----------|
| `null` (서버 연결 전) | 서버 연결 중... (콜드스타트 시 최대 3~5분 소요) |
| `history` | 세션 기록 불러오는 중... |
| `rag` | 연구 일지 조회 중... |
| `prompt` | 문서 분석 중... |
| `llm` | 답변 생성 중... (LLM 웜업 시 시간 소요) |
| `persist` | 기록 저장 중... |
